[stars-shield]: https://img.shields.io/github/stars/karanverma96/CShell.svg?style=for-the-badge
[stars-url]: https://github.com/karanverma96/CShell/stargazers
...
<br />
<p align="center">
  <a href="https://github.com/karanverma96/CShell">
    <img src="images/logo.png" alt="CShell" width="150" height="150">
  </a>
  ...
  <p align="center">
    ...
    <br />    
    <a href="https://github.com/karanverma96/CShell/issues">Report Bug</a>
    ·
    <a href="https://github.com/karanverma96/CShell/issues">Request Feature</a>
  </p>
</p>
...
CShell is a custom Unix-type shell written in C. It emulates basic Bash specifications like batch-processing(shell scripts), piping, signal-handling, I/O redirection, etc and the history feature inspired from ***ksh*** and ***csh***. Read more about Bash [here](https://en.wikipedia.org/wiki/Bash_(Unix_shell)).
...
The directory from which CShell is invoked is the home directory, indicated by `~`. If the user executes `cd` (change dir), the corresponding change is reflected in CShell as well.
...
### ***Batch-processing of Commands: CShell Scripting***

1. CShell supports processing of multiple batch files. They should be provided as space-seperated arguments while running the executable as: 

  `./CShell <file-name1> <file-name2>`
...
3. `HISTN` - Lists last "N" commands executed by CShell. If the number of commands is less than "N", it prints only the available number of commands. 
...
7. `STOP` - Terminates CShell.

8. `pid` - Prints the process id of the CShell process.

9. `pid current` - Lists the process ids of the processes that were spawned by CShell and are currently active.

10. `pid all` - Lists the process ids of all commands that were executed so far by CShell (including the currently executing processes).
...
1. **Foreground processes**: Executing any command normally(in the foreground) implies that CShell will wait for the process to complete and regain control when the process exits.

2. **Background processes**: Any command invoked with a `&` argument is treated as a background command. This implies that CShell will spawn the process and proceed(won't wait for the process to exit). When the background process exits, an appropriate indication message is shown to the user.
...
* Run the executable with/without batch file arguments (`./CShell`).
